#!/usr/bin/env bash
# ==========================================================
# Cloudflare DDNS - cfautochangeip_awsjp.sh
#
# 单文件版：
#   1. 直接执行 ./cfautochangeip_awsjp.sh = 自动安装 / 覆盖 / 重启服务
#   2. 安装后执行 txcfddns = 打开管理面板
#   3. CF_API_TOKEN / ZONE_ID 已内置在本文件；默认以脚本内置值为准
#   4. 主域名已改为 894454.xyz，子域名保留 awsjp1 / awsjp1-v6，并新增 awsjp1v4v6 双栈合并记录
...
SUB_DOMAIN="${SUB_DOMAIN:-awsjp1}"
FULL_DOMAIN="${SUB_DOMAIN}.${DOMAIN}"

RECORD_TYPE="${RECORD_TYPE:-A}"
RECORD_COMMENT="${RECORD_COMMENT:-awsjp1}"

# IPv6 备用测试记录：默认更新 awsjp1-v6.894454.xyz 的 AAAA 记录。
# 若机器没有 IPv6 出口，只会记录日志并跳过，不影响 awsjp1 IPv4。
V6_ENABLE="${V6_ENABLE:-1}"
V6_SUB_DOMAIN="${V6_SUB_DOMAIN:-awsjp1-v6}"
V6_FULL_DOMAIN="${V6_SUB_DOMAIN}.${DOMAIN}"
V6_RECORD_TYPE="${V6_RECORD_TYPE:-AAAA}"
V6_RECORD_COMMENT="${V6_RECORD_COMMENT:-awsjp1-v6}"
# 可选：指定优先读取 IPv6 的网卡，例如 eth0。留空则自动扫描所有网卡。
V6_INTERFACE="${V6_INTERFACE:-}"

# IPv4 + IPv6 合并双栈记录：默认同时维护 awsjp1v4v6.894454.xyz 的 A 和 AAAA。
# A 记录使用当前公网 IPv4；AAAA 记录使用当前公网 IPv6。
MERGED_ENABLE="${MERGED_ENABLE:-1}"
MERGED_SUB_DOMAIN="${MERGED_SUB_DOMAIN:-awsjp1v4v6}"
...
MERGED_A_RECORD_COMMENT="${MERGED_A_RECORD_COMMENT:-awsjp1v4v6-A}"
MERGED_AAAA_RECORD_COMMENT="${MERGED_AAAA_RECORD_COMMENT:-awsjp1v4v6-AAAA}"
...
INSTALL_PATH="/usr/local/sbin/cfautochangeip_awsjp.sh"
...
show_status() {
  ...
  echo
  echo "相关进程："
  ps aux | grep -Ei '[t]xcfddns|[c]fddns|[c]loudflare|awsjp1|awsjp1-v6|awsjp1v4v6' || true
}
...
show_config() {
  ...
  echo "7. 主域名固定默认 894454.xyz，保留 awsjp1 和 awsjp1-v6，并新增 awsjp1v4v6 合并双栈记录。"
  echo "8. awsjp1v4v6 会同时维护 A 和 AAAA：A 使用公网 IPv4，AAAA 使用公网 IPv6。"
